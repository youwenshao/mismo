...
export class SecurityResponder {
  ...
  private async handleSshFailure(
    studioId: string,
    event: SecurityEvent,
    state: MonitorState,
  ): Promise<void> {
    ...
    await this.alertRouter.send(
      'P0',
      'SECURITY',
      `Unauthorized SSH attempt on ${studioId}`,
      `Failed SSH from ${event.sourceIp}. Banning IP via pf.`,
      studioId,
    )
  ...
  private async handleOutboundAnomaly(
    studioId: string,
    event: SecurityEvent,
    state: MonitorState,
  ): Promise<void> {
    ...
    await this.alertRouter.send(
      'P0',
      'SECURITY',
      `Suspicious outbound traffic on ${studioId}`,
      event.details,
      studioId,
    )
    state.recordAlert(alertKey, 'P0')
  ...
  private async handleCredentialExpiry(event: SecurityEvent, state: MonitorState): Promise<void> {
    ...
    await this.alertRouter.send(
      'P1',
      'SECURITY',
      `Credential rotation needed: ${event.service}`,
      event.details,
    )

...
export class BoundaryMapper implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Boundary Mapper',
    name: 'boundaryMapper',
    icon: 'fa:map',
    group: ['transform'],
    version: 1,
    description: 'Maps module boundaries and coupling points in a repository',
    defaults: {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        const response = await this.helpers.request({
          ...
          body: { cloneDir, astData, surgeryId, operation: 'boundary' },
          json: true,
        })

...
function parseChoicesFromContent(content: string): Choice[] | null {
...
function ChatPageInner() {
  ...
  const [priceEstimates, setPriceEstimates] = useState<Map<number, PriceEstimate>>(new Map())
  const [parsedChoices, setParsedChoices] = useState<Map<number, Choice[]>>(new Map())
  ...
  async function handleEditMessage(messageIndex: number, newContent: string) {
    ...
    try {
      const res = await fetch(`/api/interview/session/${sessionId}/rewind`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="h-dvh flex flex-col bg-white">
      <header
        className="flex items-center justify-between px-6 py-4 shrink-0 bg-white/80 backdrop-blur-md z-50"
        style={{
      ...
      {mode === 'voice' ? (
        <>
          <div className="flex-1 flex flex-col items-center justify-center">
            <button
              onClick={() => alert('Voice mode requires LiveKit configuration')}
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 pb-16">
          <h1 className="text-3xl font-semibold text-center mb-8">What can I help you build?</h1>
    ...
    <div className="flex justify-center gap-1 mt-3">
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-full transition-colors ${
          mode === 'text' ? 'bg-gray-100 text-gray-900' : 'text-gray-400 hover:text-gray-600'
        }`}
      >
      ...
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-full transition-colors ${
          mode === 'voice' ? 'bg-gray-100 text-gray-900' : 'text-gray-400 hover:text-gray-600'
        }`}
      >
        <Mic size={12} />
        Voice
